...
const kızroleID = await data.fetch(`kız.${message.guild.id}`);
...
**Örnek:**
\`\`\`${client.ayarlar.prefix}kız @etiket
${client.ayarlar.prefix}kız @etiket blabla 123

${client.ayarlar.prefix}kız 686185592899633200
${client.ayarlar.prefix}kız 686185592899633200 blabla 123\`\`\``)).then(a => a.delete({timeout: 10000}));


let member = message.mentions.members.first();
let kız = message.guild.roles.cache.get(kızroleID);
let yetkili = message.guild.roles.cache.get(yetkiliroleID);
let kayıtsız = message.guild.roles.cache.get(kayıtsızroleID);
if(!kız||!kayıtsız||!yetkili) return;
let isim;
if(args[1]) {
...
member.roles.add(kız.id);
member.roles.remove(kayıtsız.id);
data.add(`say.kız.${message.author.id}.${message.guild.id}`, 1);
...
.setDescription(`${member} **Kullanıcısına ${kız} rolü verildi.**

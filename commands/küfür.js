...
return message.channel.send(nn.setTitle(`İşte bu kadar!`).setDescription('<a:emoji_6:814954985481175120> Küfür kısıtlaması başarıyla açıldı.')).then(a => a.delete({timeout: 10000}));
} else if(args[0] === 'kapat') {
data.delete(`küfür.${message.guild.id}`);
return message.channel.send(nn.setTitle('İşte bu kadar!').setDescription('<a:emoji_6:814954985481175120> Küfür kısıtlaması başarıyla kapatıldı.')).then(a => a.delete({timeout: 10000}));
}
};
exports.conf = {
  enabled: true,
